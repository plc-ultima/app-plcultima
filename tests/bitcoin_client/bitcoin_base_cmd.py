...
class BitcoinBaseCommand:
    ...
    def get_coin_version(self) -> Tuple[int, int, str, str]:
        """Get coin information depending on Bitcoin app fork.

        Returns
        -------
        Tuple[int, int, str, str]
            A tuple (p2pkh_pref, p2sh_prefix, coin_name, coin_ticker).
        ...
        # response = p2pkh_prefix (2) || p2sh_prefix (2) ||
        #            len(coin_name) (1) || coin_name (var) ||
        #            len(coin_ticker) (1) || coin_ticker (var)
        offset: int = 0
        p2pkh_prefix: int = int.from_bytes(response[offset:offset + 3], byteorder="big")
        offset += 3
        p2sh_prefix: int = int.from_bytes(response[offset:offset + 3], byteorder="big")
        offset += 3
        ...
        return p2pkh_prefix, p2sh_prefix, coin_name, coin_ticker
