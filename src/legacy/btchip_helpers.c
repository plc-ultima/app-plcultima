...
unsigned short btchip_public_key_to_encoded_base58(
    ...
    unsigned short outlen, unsigned int version,
    unsigned char alreadyHashed) {
    unsigned char tmpBuffer[35];

    unsigned char versionSize = version > 65535 ? 3 : version > 255 ? 2 : 1;
    size_t outputLen;
    ...
    if (!alreadyHashed) {
        ...
        if (version > 65535) {
            tmpBuffer[0] = (version >> 16) & 0xFF;
            tmpBuffer[1] = (version >> 8) & 0xFF;
            tmpBuffer[2] = version & 0xFF;
        } else if (version > 255) {
